...
final class ShoppingViewModel {
    ...
    private let tagList = Observable.just(["사과", "생크림빵", "고구마", "방울토마토", "아이패드 케이스", "제로콜라", "치킨", "야구직관"])
    ...
    func transform(_ input: Input) -> Output? {
        ...
        input.selectedTag
            .bind(with: self) { owner, tagName in
                owner.updateItems(items: items, updateType: .add(itemName: tagName))
            }.disposed(by: disposeBag)
        
        let output = Output(items: items, addedItemTrigger: addedItemTrigger,
                            selectedItem: input.selectedItem, searchBtnTapped: searchBtnTapped, tagList: tagList)
        return output
    }
